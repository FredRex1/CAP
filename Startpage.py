...
conn = pymssql.connect(
    host="localhost", database="test", user="user", password="pass-word", charset="utf8"
)
cursor = conn.cursor()
app = Flask(__name__)
...
def accountPageEdit():
    ...
    return render_template(
        "accountPageEdit.html", userrole=session["rolename"], accountinfo=accountinfo
    )
...
def dashboard():
    ...
    cursor.execute(
        "SELECT [ScheduleID] FROM [test].[dbo].[Recipients] WHERE [UserID]= %s AND [Active] = 1",
        (userid),
    )
...
def myFiles():
    ...
    return render_template(
        "myFiles.html", username=session["username"], fileinfo=htmlText
    )
...
def report():
    ...
    for row in result:
        if counter == 3:
            ...
            box += (
                '<div class="file-box">  <div class="file"> <a href="%s"> <span class="corner"></span> <div class="icon"> <i class="fa fa-file"></i> </div> <div class="file-name"> %s \
                        <br> <small>Desc: %s</small>  </div> </a> </div> </div>'
                % (filepath, str(row[0]), str(row[2]))
            )
    ...
    return render_template(
        "report.html", username=session["username"], fileinfo=htmlText
    )
...
def scheduling():
    ...
    # =========================================================
    cursor.execute(
        "INSERT INTO [test].[dbo].[ScheduleTask] VALUES ( %s, %s, %s);",
        (ScheduleTaskId, Descrption, 0),
    )
    conn.commit()
    cursor.execute(
        "SELECT [FileID] FROM [test].[dbo].[File] WHERE [FileName]= %s", (file)
    )
    row = cursor.fetchone()
    cursor.execute(
        "INSERT INTO [test].[dbo].[SchedulePeriod] VALUES ( %s, %s, %s, %s, %s, %s);",
        (
            ScheduleId,
            ScheduleTaskId,
            Hostpital_System,
    ...
    for R_user in user:
        ...
        cursor.execute(
            "INSERT INTO [test].[dbo].[Recipients] VALUES ( %s, %s, %s, %s);",
            (ReciptenceId, ScheduleId, R_user, 0),
        )
    # ====================================================================
